...
public class CommentService {
    ...
    public ApiResponseDto<CommentResponseDto> createComment(Long id, CommentRequestDto requestDto, User user) {
        ...
        Long parentCommentId = requestDto.getParentCommentId(); // 원댓글 조회

        Comment comment = Comment.of(requestDto, board.get(), user); // 작성하여 저장할 comment 변수 선언

        if (parentCommentId == null) {  // parentComment 가 없다면 -> 원댓글을 작성할 경우
            commentRepository.save(comment);    // 바로 저장
            return ResponseUtils.ok(CommentResponseDto.from(comment));
        }

        // parentComment 가 있다면 -> 대댓글을 작성하는 경우
        // parent comment 에 childComment 를 추가
        Comment parentComment = commentRepository.findById(parentCommentId)
                .orElseThrow(() -> new RestApiException(ErrorType.NOT_FOUND_WRITING)); // parentComment가 있는데 댓글이 없는 경우
        parentComment.addChildComment(comment); // parentComment 에 childComment 추가
        commentRepository.save(comment); // db 에 반영
